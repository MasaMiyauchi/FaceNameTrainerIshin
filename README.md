...
このリポジトリには、環境設定を自動化するスクリプトが含まれています。
...
#### PHP スクリプト (すべてのプラットフォーム)

```
php setup.php
```

これらのスクリプトは以下の設定を行います：

1. 必要なディレクトリ（assets/data/、assets/faces/）の作成
2. 適切な権限設定
3. .env ファイルの設定
4. PHP 依存関係のチェックと必要に応じたインストール

スクリプト実行後、Stability AI API キーを .env ファイルに設定する必要があります。
